**Twitter Sentiment Analysis:** For this project, we will attempt to build a model to detect how positive or negative the sentiment of tweets is. Given the breadth of subjects addressed in tweets, such a model will be well-suited for general use across different subjects. Although the cost of this generalization is apt to be a lack of power when compared to more specialized models, the ease-of-applicability of such a model is valuable in that it can be utilized to gain a rudimentary big-picture view of user sentiment prior to the development of a specialized model for issues relating to a particular industry or organization.
